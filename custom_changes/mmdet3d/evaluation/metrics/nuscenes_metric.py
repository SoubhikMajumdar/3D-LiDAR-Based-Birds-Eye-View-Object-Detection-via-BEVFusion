...
class NuScenesMetric(BaseMetric):
    ...
    def _evaluate_single(
        ...
        # PATCH START: Imports for monkeypatch
        from nuscenes.eval.detection.evaluate import (
            is_predefined_split, load_prediction, load_gt, get_samples_of_custom_split,
            load_prediction_of_sample_tokens, load_gt_of_sample_tokens, add_center_dist,
            filter_eval_boxes
        )
        ...
        def patched_init(self,
            ...
            if is_predefined_split(split_name=eval_set):
                self.pred_boxes, self.meta = load_prediction(self.result_path, self.cfg.max_boxes_per_sample, DetectionBox,
                                                            verbose=verbose)
                # PATCH: Do not load full GT yet.
            else:
                # Custom split logic
                sample_tokens_of_custom_split = get_samples_of_custom_split(split_name=eval_set, nusc=nusc)
            ...
            # PATCH: Force load GT only for the tokens we have predictions for
            # This handles the partial dataset case where we only processed a subset
            pred_tokens = list(self.pred_boxes.sample_tokens)
            print(f"PATCH: Loading GT for {len(pred_tokens)} predicted samples only (Partial Dataset Mode).")
            self.gt_boxes = load_gt_of_sample_tokens(nusc, pred_tokens, DetectionBox, verbose=verbose)

            # Assertion should now pass automatically
